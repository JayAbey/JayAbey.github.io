...
/**********Header Logo**********/
function bulmapress_custom_logo_setup()
{
    $defaults = array(
        'height' => 100,
        'width' => 400,
...
add_action('after_setup_theme', 'bulmapress_custom_logo_setup');
...
function bulmapress_home_link($class)
{
    ...
    if ( has_custom_logo() ) {
        $output = '<a href="' . esc_url( home_url( '/' ) ) . '" class="' . $class . '" rel="home">' . '<img src="'. esc_url( $logo[0] ) .'">' . '</a>';
    } else {
